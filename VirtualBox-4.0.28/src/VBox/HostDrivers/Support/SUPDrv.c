...
int VBOXCALL supdrvInitDevExt(PSUPDRVDEVEXT pDevExt, size_t cbSession)
{
    ...
    if (RT_SUCCESS(rc))
        rc = RTSpinlockCreate(&pDevExt->hSessionHashTabSpinlock);
    if (RT_SUCCESS(rc))
                                        ...
                                        void *pvUser)
{
    ...
    RTSpinlockAcquireNoInts(pDevExt->hSessionHashTabSpinlock, &Tmp);
    ...
    if (pCur)
    {
        ...
        if (pCur)
        {
            RTSpinlockReleaseNoInts(pDevExt->hSessionHashTabSpinlock, &Tmp);
            if (pCur == pSession)
            {
                Assert(pSession->fInHashTable);
                AssertFailed();
